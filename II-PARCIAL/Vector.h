...
public:
	...
	/**
	  * @brief ordenamientoInsercionBinaria
	  * @return void
	  */
	...
	/**
	  * @brief ordenarArregloPorSeleccion
	  * @return void
	  */
	void ordenarArregloPorSeleccion();
	/**
	  * @brief ordenamientoGnome
	  * @return void
	  */
	void ordenamientoGnome();
	/**
	  * @brief ordenamientoBurbuja
	  * @param i
	  */
...
template <class T>
void Vector<T>::shell(){
    int i = 0 , j = 0 , k = 0 , incremento = 0 , aux = 0;
    incremento = tamanio/2;
    while( incremento > 0){
        for( i = incremento + 1 ; i <= tamanio ; i++){
            j = i - incremento ;
            while( j > 0 ){
                if( * (dato + j ) >= *( dato + ( j + incremento ) ) ){
                    aux = *( dato + j );
                    *( dato + j ) = *( dato + ( j + incremento ) );
                    *( dato  + ( j + incremento ) ) = aux;
                }
                else{
                        j = 0 ;
                    }
...
void Vector<T>::heapSort(){
	int temp = 0;
	int item = 0 ;
	for(int k = tamanio ; k > 0 ; k--)
    {
        for(int i = 1 ; i <= k ; i++)
        {
            item = *( dato + i);
            int j = i/2;
            while( j > 0 && *( dato + j) < item)
            {
                *( dato + i )= *( dato + j );
                i = j ;
                j = j/2 ;
...
template<class T>
void Vector<T>::ordenamientoGnome()
{
